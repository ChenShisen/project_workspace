...
try:
    import mha_optimizer_cpp
except ImportError:
    warnings.warn(
        "Could not import 'mha_optimizer_cpp'. "
        "Please build the C++ extension by running 'pip install .' in the project root."
    )
    exit()
...
class OptimizedMHA(nn.Module):
    ...
    def forward(self, hidden_states_packed, cu_seqlens):
        return mha_optimizer_cpp.forward(
...
def run_benchmark(dtype_str):
    ...
    print("\n" + "="*60)
    print(f"        MHA BENCHMARK FOR {dtype_str.upper()}")
    print("="*60)
    ...
    baseline_model = StandardMHA(config).to(device).eval()
    optimized_model = OptimizedMHA(config).to(device).eval()
    ...
    print("\n--- Running Baseline (Padded PyTorch)...")
    with torch.no_grad():
    ...
    print("\n--- Running Optimized MHA (Packed, CUTLASS + Custom Kernel)...")
    with torch.no_grad():
    ...
    print(f"Optimized Latency: {optimized_latency:.4f} ms")
